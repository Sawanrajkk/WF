...
from dunetuf.features.job.job_configuration.job_configuration import JobConfiguration
...
from dunetuf.engine.maia.supplies.phMaia import phMaia
...
from dunetuf.utils.tuf_root_logger import configure_root_logger
...
def spice(request):
    """Spice fixture."""
    target_platform = device_instance().target_platform   
    logging.info(f"Target Platform detected: {target_platform}")
    if target_platform == TargetPlatform.ARES:
        logging.info("Ares platform detected - skipping Dune-specific setup")
        yield
        return
    elif target_platform == TargetPlatform.DUNE:
        spice = Spice(request.config.option.target, request.config.option.qmltest_port, request.config.option.screencapture)
        spice.wait_ready()
        yield spice
    else:
...
def cdm(request):
    ...
    target_platform = device_instance().target_platform   
    logging.info(f"Target Platform detected: {target_platform}")
    if target_platform in (TargetPlatform.ARES, TargetPlatform.DUNE):
...
@pytest.fixture
def cleanup_backup_restore(request):
    """Cleanup backup files after test execution."""
    yield
    # Access module-level backup_restore instance if it exists
    test_module = request.node.module
    if hasattr(test_module, 'backup_restore') and test_module.backup_restore:
        test_module.backup_restore.cleanup()

@pytest.fixture
def cleanup_import_export(request):
    """Cleanup export files after test execution."""
    yield
    # Access module-level import_export instance if it exists
    test_module = request.node.module
    if hasattr(test_module, 'import_export') and test_module.import_export:
        test_module.import_export.cleanup()
